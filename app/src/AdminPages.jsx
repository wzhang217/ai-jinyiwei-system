...
function NotificationSettings({ role = "admin", notifications, loading = false, onChanged, onToast }) {
  ...
  return <SectionCard title="通知设置" description="只针对数据质量和协作问题发送通知">{values.length ? <><div className="notification-list">{values.map((item) => <NotificationRow key={item.key} label={item.label} enabled={item.enabled} onToggle={() => toggle(item.key)} />)}</div><button className="outline-button" disabled={!canEdit || loading || saving} onClick={() => void save()}><CheckCircle size={16} />{saving ? "保存中…" : canEdit ? "保存通知设置" : "老板可编辑"}</button></> : <EmptyState title={loading ? "正在读取通知设置…" : "暂无通知设置"} />}</SectionCard>;
}
...
function IntegrationSettings({ role = "admin", integrations, loading = false, onChanged, onToast }) {
  ...
  return <SectionCard title="数据与集成" description="第三方数据源默认关闭，启用前需要管理员授权">{items.length ? <><div className="integration-grid">{items.map((item) => <IntegrationCard key={item.key} icon={item.key === "browser_extension" ? <Browser size={21} /> : item.key === "project_tools" ? <Code size={21} /> : item.key === "collaboration" ? <ChatCircleDots size={21} /> : <Database size={21} />} title={item.title} detail={item.detail} status={item.status} enabled={item.enabled} onClick={() => toggle(item.key)} />)}</div><button className="outline-button" disabled={!canEdit || loading || saving} onClick={() => void save()}><CheckCircle size={16} />{saving ? "保存中…" : canEdit ? "保存集成设置" : "老板可编辑"}</button></> : <EmptyState title={loading ? "正在读取数据集成…" : "暂无数据集成"} />}</SectionCard>;
}
