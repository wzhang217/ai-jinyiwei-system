...
function TeamsPage({ role, principal, target, liveRecords, onNavigate, onToast }) {
  ...
  const teamTrendDays = Array.from({ length: 7 }, (_, index) => shanghaiDateAtStart(6 - index));
  const teamTrend = teamTrendDays.map((date) => teamRecords.filter((record) => shanghaiDayKey(record.started_at || record.startedAt || record.period_start || 0) === shanghaiDayKey(date)).reduce((sum, record) => sum + Number(record.durationSeconds || record.duration_seconds || 0), 0));
  const maxTeamTrend = Math.max(...teamTrend, 1);
  const teamTrendLabels = teamTrendDays.map((date) => date.toLocaleDateString("zh-CN", { timeZone: SHANGHAI_TIME_ZONE, weekday: "short" }).replace("周", "周"));
...
function EmployeesPage({ role, principal, query, target, liveRecords, onNavigate, onToast }) {
  ...
  const refreshDirectory = async () => {
    if (!agentApiEnabled) {
      onToast("连接服务端后才能同步员工目录");
      return;
    }
    setDirectoryRefreshing(true);
    setDirectoryError("");
    try {
      const [employees, devices] = await Promise.all([getLiveEmployees(), getLiveDevices()]);
      setLiveEmployees(employees);
      setLiveDevices(devices);
      onToast(`员工目录已刷新（${employees.length} 位成员）`);
    } catch (error) {
  ...
  const exportSelectedRecords = async (record) => {
    const recordsToExport = record ? [record] : selectedRecords;
    if (!recordsToExport.length) {
      onToast("暂无可导出的个人 Memory Summary");
      return;
    }
    try {
      if (agentApiEnabled) await exportLiveAudit(recordsToExport.map((item) => item.id));
      downloadRecordsMarkdown(recordsToExport);
      onToast("已导出个人 Memory Summary");
    } catch (error) {
...
function DiagnosticRow({ icon, title, detail }) { return <div className="diagnostic-row">{icon}<span><strong>{title}</strong><small>{detail}</small></span></div>; }
function DeviceRow({ device, onNavigate }) { return <div className="device-detail-row"><span className="device-icon large"><Monitor size={21} /></span><span><strong>{device.name}</strong><small>{device.os} · Agent {device.agent}</small></span><StatusPill status={device.status} /><button className="outline-button" onClick={() => onNavigate("devices", device.id)}><Wrench size={16} />打开设备详情</button></div>; }
function DeviceDrawer({ device, role, onClose, onToast, onNavigate }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="memory-drawer device-drawer"><div className="drawer-header"><span className="page-eyebrow">DEVICE DETAIL</span><button className="drawer-close" onClick={onClose}>×</button><h2>{current.hostname || current.name}</h2><p>{current.employee_name || current.user} · {current.os_version || current.os} · Agent {current.agent_version || current.agent}</p></div><div className="drawer-content"><div className="device-status-banner"><StatusPill status={disabled ? "locked" : current.status} /><span>最近心跳 {current.last_heartbeat_at ? formatShanghaiTime(current.last_heartbeat_at) : current.heartbeat}</span></div>{error && <div className="error-box">设备操作失败：{error.message || error}</div>}<DrawerSection title="工作会话"><div className="detail-definition"><span>当前状态</span><strong>{disabled ? "已停用" : current.session || (current.status === "online" ? "active" : "offline")}</strong><span>离线缓存</span><strong>{current.queued_events ?? current.cache ?? 0} 条</strong><span>最近错误</span><strong>{(current.queued_events ?? current.cache ?? 0) > 0 ? `${current.queued_events ?? current.cache} 条待上传` : "无"}</strong><span>最近事件</span><strong>{detail?.events?.length ?? "读取中…"}</strong></div></DrawerSection><DrawerSection title="设备操作"><div className="drawer-button-list"><button onClick={() => onToast("重新注册需在 Agent 托盘中输入新的注册码") }><Fingerprint size={17} />重新注册 Agent</button><button onClick={() => onNavigate("permissions", "policy")}><SlidersHorizontal size={17} />查看采集策略</button><button disabled={busy} onClick={() => void changeStatus(disabled)}>{disabled ? <CheckCircle size={17} /> : <LockKey size={17} />}{busy ? "处理中…" : disabled ? "启用设备" : "停用设备"}</button></div></DrawerSection></div></aside></div>;
}

function PermissionsPage({ role, principal, target, onToast }) {
  const canEdit = role === "admin";
  const [tab, setTab] = useState(target === "policy" ? "采集策略" : target === "retention" ? "保留策略" : "角色");
...
function ScopePolicy({ role = "admin", onToast }) {
  const fallback = permissionRoles.map((item, index) => ({ role: ["admin", "manager", "employee"][index], label: item.role, scope: item.scope, detail: item.description }));
  const [roles, setRoles] = useState(agentApiEnabled ? [] : fallback);
  const [loading, setLoading] = useState(agentApiEnabled);
  const [error, setError] = useState("");
  useEffect(() => {
    if (!agentApiEnabled) return undefined;
  ...
  return <SectionCard title="数据范围" description="老板、高管、员工的数据范围由服务端 RBAC 强制执行，页面仅展示不可绕过的权限边界">{roles.length ? <div className="scope-list">{roles.map((item) => <div className="scope-row" key={item.role}><span><strong>{item.label}</strong><small>{item.detail}</small></span><b>{loading ? "读取中…" : item.scope}</b><CaretDown size={15} /></div>)}</div> : <EmptyState title={loading ? "正在读取数据范围…" : "暂无真实权限范围"} />}{error && <div className="error-box">数据范围读取失败：{error}</div>}<small className="policy-hint">历史、设备、Memory Summary、审计和 History Skill 查询均经过服务端范围过滤；老板可管理策略，高管和员工不能越权。</small></SectionCard>;
}
...
function SettingsPage({ role, onToast }) {
  ...
  const [error, setError] = useState("");
  const settingsLoading = agentApiEnabled && !settings && !error;
  ...
  return <div className="page-content"><PageHeader eyebrow="ORGANIZATION SETTINGS" title="设置" description="管理企业资料、工作会话、活动分类、AI、通知和数据集成。" meta={roleLabel[role]} action={<span className="scope-pill">{agentApiEnabled ? settings ? "已连接服务端" : "正在读取…" : "演示模式"}</span>} />{error && <div className="error-box">设置读取失败：{error}</div>}<Tabs tabs={tabs} active={tab} onChange={setTab} />{tab === "企业资料" && <SettingsForm role={role} settings={settings?.organization} loading={settingsLoading} onChanged={(organization) => updateSettings({ organization })} onToast={onToast} />}{tab === "工作会话" && <PolicyEditor role={role} onToast={onToast} />}{tab === "活动分类" && <CategorySettings role={role} categories={settings?.categories} loading={settingsLoading} onChanged={(categories) => updateSettings({ categories })} onToast={onToast} />}{tab === "AI 设置" && <AiSettings role={role} settings={settings?.organization} loading={settingsLoading} onChanged={(organization) => updateSettings({ organization })} onToast={onToast} />}{tab === "通知" && <NotificationSettings role={role} notifications={settings?.notifications} loading={settingsLoading} onChanged={(notifications) => updateSettings({ notifications })} onToast={onToast} />}{tab === "数据与集成" && <IntegrationSettings role={role} integrations={settings?.integrations} loading={settingsLoading} onChanged={(integrations) => updateSettings({ integrations })} onToast={onToast} />}{tab === "安全合规" && <ComplianceSettings role={role} onToast={onToast} />}</div>;
}
function SettingsForm({ role = "admin", settings, loading = false, onChanged, onToast }) {
  const canEdit = role === "admin";
  const sourceMissing = agentApiEnabled && !settings;
  const [draft, setDraft] = useState({ company_name: agentApiEnabled ? "" : "锦衣卫科技", default_language: agentApiEnabled ? "" : settingsData.language, timezone: agentApiEnabled ? "" : settingsData.timezone, retention: agentApiEnabled ? "" : settingsData.retention });
  const [saving, setSaving] = useState(false);
...
function CategorySettings({ role = "admin", categories, loading = false, onChanged, onToast }) {
  const canEdit = role === "admin";
  ...
  const [items, setItems] = useState(agentApiEnabled ? [] : fallback);
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (categories) setItems(categories); }, [categories]);
  ...
  return <SectionCard title="活动分类" description="分类用于摘要和趋势解释，不自动代表绩效结论">{items.length ? <><div className="category-list">{items.map((item) => <div className="category-row" key={item.id}><span className={`category-dot ${item.color}`} /><span><input className="inline-setting-input" disabled={!canEdit} value={item.label} onChange={(event) => updateItem(item.id, "label", event.target.value)} /><small><input className="inline-setting-input detail" disabled={!canEdit} value={item.detail} onChange={(event) => updateItem(item.id, "detail", event.target.value)} /></small></span><button type="button" className="toggle-button" disabled={!canEdit} onClick={() => updateItem(item.id, "enabled", !item.enabled)}><span className={`toggle ${item.enabled ? "on" : ""}`}><i /></span></button></div>)}</div><button className="outline-button" disabled={!canEdit || loading || saving} onClick={() => void save()}><CheckCircle size={16} />{saving ? "保存中…" : canEdit ? "保存分类" : "老板可编辑"}</button></> : <EmptyState title={loading ? "正在读取活动分类…" : "暂无活动分类"} />}</SectionCard>;
}
function NotificationSettings({ role = "admin", notifications, loading = false, onChanged, onToast }) {
  const canEdit = role === "admin";
  ...
  const [values, setValues] = useState(agentApiEnabled ? [] : fallback);
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (notifications) setValues(notifications); }, [notifications]);
  ...
  return <SectionCard title="通知设置" description="只针对数据质量和协作问题发送通知">{values.length ? <><div className="notification-list">{values.map((item) => <NotificationRow key={item.key} label={item.label} enabled={item.enabled} disabled={!canEdit} onToggle={() => toggle(item.key)} />)}</div><button className="outline-button" disabled={!canEdit || loading || saving} onClick={() => void save()}><CheckCircle size={16} />{saving ? "保存中…" : canEdit ? "保存通知设置" : "老板可编辑"}</button></> : <EmptyState title={loading ? "正在读取通知设置…" : "暂无通知设置"} />}</SectionCard>;
}
function NotificationRow({ label, enabled, disabled = false, onToggle }) { return <div className="notification-row"><span><strong>{label}</strong><small>{enabled ? "当前开启" : "当前关闭"}</small></span><button type="button" className="toggle-button" disabled={disabled} aria-label={`${label}：${enabled ? "关闭" : "开启"}`} onClick={onToggle}><span className={`toggle ${enabled ? "on" : ""}`}><i /></span></button></div>; }
function IntegrationSettings({ role = "admin", integrations, loading = false, onChanged, onToast }) {
  const canEdit = role === "admin";
  ...
  const [items, setItems] = useState(agentApiEnabled ? [] : fallback);
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (integrations) setItems(integrations); }, [integrations]);
  ...
  return <SectionCard title="数据与集成" description="第三方数据源默认关闭，启用前需要管理员授权">{items.length ? <><div className="integration-grid">{items.map((item) => <IntegrationCard key={item.key} icon={item.key === "browser_extension" ? <Browser size={21} /> : item.key === "project_tools" ? <Code size={21} /> : item.key === "collaboration" ? <ChatCircleDots size={21} /> : <Database size={21} />} title={item.title} detail={item.detail} status={item.status} enabled={item.enabled} disabled={!canEdit} onClick={() => toggle(item.key)} />)}</div><button className="outline-button" disabled={!canEdit || loading || saving} onClick={() => void save()}><CheckCircle size={16} />{saving ? "保存中…" : canEdit ? "保存集成设置" : "老板可编辑"}</button></> : <EmptyState title={loading ? "正在读取数据集成…" : "暂无数据集成"} />}</SectionCard>;
}
function IntegrationCard({ icon, title, detail, status, enabled, disabled = false, onClick }) { const display = status === "connected" ? "已连接" : status === "partial" ? "部分连接" : status === "preparing" ? "准备中" : "未连接"; return <button type="button" className={`integration-card${disabled ? " disabled" : ""}`} disabled={disabled} onClick={onClick}><span className="integration-icon">{icon}</span><span><strong>{title}</strong><small>{detail} · {enabled ? "已启用" : "已停用"}</small></span><StatusPill status={status === "connected" || enabled && status === "partial" ? "online" : status === "partial" ? "meeting" : "pending"} /><b>{display}</b></button>; }
