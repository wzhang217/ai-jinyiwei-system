...
function PermissionsPage({ role, principal, target, onToast }) {
  ...
  return <div className="page-content"><PageHeader eyebrow="ACCESS CONTROL" title="权限" description="管理老板、高管、员工三种角色、数据范围和采集策略。" meta="RBAC 已启用" action={<span className="scope-pill"><Key size={17} />固定三种角色</span>} />{error && <div className="error-box">权限配置读取失败：{error.message || error}</div>}<Tabs tabs={tabs} active={tab} onChange={setTab} />{tab === "账号" && canEdit && <AccountManagement onToast={onToast} />}{tab === "角色" && <SectionCard title="角色权限" description="系统只保留老板、高管、员工三种角色，权限由服务端强制执行"><div className="role-list">{roleItems.map((item) => <div className="role-row" key={item.role}><span className="role-icon"><Key size={18} /></span><span><strong>{item.role}</strong><small>{item.scope} · {item.users || "—"} 人 · {item.description || item.detail}</small></span></div>)}</div></SectionCard>}{tab === "组织关系" && <OrgTree principal={principal} onToast={onToast} />}{tab === "数据范围" && <ScopePolicy role={role} onToast={onToast} />}{tab === "采集策略" && <PolicyEditor role={role} onToast={onToast} />}{tab === "应用/网站排除" && <ExclusionPolicy role={role} onToast={onToast} />}{tab === "保留策略" && <RetentionPolicy role={role} onToast={onToast} />}</div>;
}
