...
function PermissionsPage({ role, target, onToast }) {
  ...
  return <div className="page-content"><PageHeader eyebrow="ACCESS CONTROL" title="权限" description="管理角色、组织关系、数据范围和采集策略。" meta="RBAC 已启用" action={<button className="outline-button" onClick={() => onToast(canEdit ? "新增角色需要连接权限服务" : "当前角色只能查看权限配置")}><Plus size={17} />新增角色</button>} /><Tabs tabs={["角色", "组织关系", "数据范围", "采集策略", "应用/网站排除", "保留策略"]} active={tab} onChange={setTab} />{tab === "角色" && <SectionCard title="角色权限" description="权限变更会记录操作者和生效时间"><div className="role-list">{permissionRoles.map((item) => <div className="role-row" key={item.role}><span className="role-icon"><Key size={18} /></span><span><strong>{item.role}</strong><small>{item.scope} · {item.users} 人 · {item.description}</small></span><button className="outline-button" onClick={() => onToast(`已打开 ${item.role} 权限详情`)}>查看</button></div>)}</div></SectionCard>}{tab === "组织关系" && <OrgTree onToast={onToast} />}{tab === "数据范围" && <ScopePolicy onToast={onToast} />}{tab === "采集策略" && <PolicyEditor role={role} onToast={onToast} />}{tab === "应用/网站排除" && <ExclusionPolicy role={role} onToast={onToast} />}{tab === "保留策略" && <RetentionPolicy role={role} onToast={onToast} />}</div>;
}
...
function ScopePolicy({ onToast }) { return <SectionCard title="数据范围" description="选择不同角色可以查看的组织范围"><div className="scope-list"><ScopeRow role="老板" value="整个企业" detail="历史、Memory Summary、设备、权限、策略和审计" onToast={onToast} /><ScopeRow role="高管" value="直属团队" detail="团队趋势、成员历史和团队 Skill" onToast={onToast} /><ScopeRow role="员工" value="本人" detail="自己的历史、Memory Summary、设备和隐私说明" onToast={onToast} /></div><button className="outline-button" onClick={() => onToast("数据范围规则已保存")}><CheckCircle size={16} />保存范围规则</button></SectionCard>; }
function ScopeRow({ role, value, detail, onToast }) { return <button type="button" className="scope-row" onClick={() => onToast(`${role}的数据范围：${value}`)}><span><strong>{role}</strong><small>{detail}</small></span><b>{value}</b><CaretDown size={15} /></button>; }
...
function SettingsPage({ role, onToast }) {
  ...
  return <div className="page-content"><PageHeader eyebrow="ORGANIZATION SETTINGS" title="设置" description="管理企业资料、工作会话、活动分类、AI、通知和数据集成。" meta={roleLabel[role]} action={<button className="primary-button" disabled={!canEdit} onClick={() => onToast(canEdit ? "设置已保存" : "当前角色只能查看设置；请切换为老板后保存")}>{canEdit ? "保存设置" : "老板可编辑"}</button>} /><Tabs tabs={["企业资料", "工作会话", "活动分类", "AI 设置", "通知", "数据与集成", "安全合规"]} active={tab} onChange={setTab} />{tab === "企业资料" && <SettingsForm role={role} onToast={onToast} />}{tab === "工作会话" && <PolicyEditor role={role} onToast={onToast} />}{tab === "活动分类" && <CategorySettings role={role} onToast={onToast} />}{tab === "AI 设置" && <SectionCard title="AI 模型适配层" description="摘要和问答必须保留 Citations，并对事件文本进行不可信数据隔离"><div className="settings-grid"><SettingField label="摘要语言" value="简体中文 / English" /><SettingField label="默认模型" value="qwen3.7-plus（服务端）" /><SettingField label="调用预算" value="30 次 / 分钟" /><SettingField label="原始内容读取" value="禁止" /></div></SectionCard>}{tab === "通知" && <NotificationSettings role={role} onToast={onToast} />}{tab === "数据与集成" && <IntegrationSettings onToast={onToast} />}{tab === "安全合规" && <ComplianceSettings onToast={onToast} />}</div>;
}
function SettingsForm({ role = "admin", onToast }) { const canEdit = role === "admin"; return <SectionCard title="企业资料" description="这些信息会出现在组织导航和 Memory Summary 权限范围中"><div className="settings-grid"><SettingField label="企业名称" value="锦衣卫科技" disabled={!canEdit} /><SettingField label="默认语言" value={settingsData.language} disabled={!canEdit} /><SettingField label="默认时区" value={settingsData.timezone} disabled={!canEdit} /><SettingField label="数据保留" value={settingsData.retention} disabled={!canEdit} /></div><button className="outline-button" disabled={!canEdit} onClick={() => onToast(canEdit ? "企业资料已保存" : "当前角色只能查看企业资料；请切换为老板后保存")}><CheckCircle size={16} />{canEdit ? "保存资料" : "老板可编辑"}</button></SectionCard>; }
function SettingField({ label, value, disabled = false }) { return <label className="setting-field"><span>{label}</span><input disabled={disabled} defaultValue={value} /></label>; }
function CategorySettings({ role = "admin", onToast }) { const canEdit = role === "admin"; return <SectionCard title="活动分类" description="分类用于摘要和趋势解释，不自动代表绩效结论"><div className="category-list"><CategoryRow color="purple" label="工作与项目" detail="开发、文档、项目管理" onToast={onToast} /><CategoryRow color="blue" label="沟通与会议" detail="企业微信、会议和邮件" onToast={onToast} /><CategoryRow color="green" label="系统与工具" detail="登录、设置、故障和同步" onToast={onToast} /><CategoryRow color="amber" label="疑似非工作" detail="购物、娱乐、求职和游戏，需人工确认" onToast={onToast} /><CategoryRow color="gray" label="未知" detail="无法可靠分类的活动" onToast={onToast} /></div><button className="outline-button" disabled={!canEdit} onClick={() => onToast(canEdit ? "分类规则已保存" : "当前角色只能查看分类；请切换为老板后保存")}><CheckCircle size={16} />{canEdit ? "保存分类" : "老板可编辑"}</button></SectionCard>; }
function CategoryRow({ color, label, detail, onToast }) { return <div className="category-row"><span className={`category-dot ${color}`} /><span><strong>{label}</strong><small>{detail}</small></span><button className="row-action" onClick={() => onToast(`${label}分类规则已打开`)}><ArrowSquareOut size={15} /></button></div>; }
function NotificationSettings({ role = "admin", onToast }) { const canEdit = role === "admin"; const [values, setValues] = useState([true, true, true, false]); const labels = ["Agent 离线超过 30 分钟", "Memory Summary 生成失败", "团队数据覆盖率低于 90%", "疑似非工作活动"]; return <SectionCard title="通知设置" description="只针对数据质量和协作问题发送通知"><div className="notification-list">{labels.map((label, index) => <NotificationRow key={label} label={label} enabled={values[index]} onToggle={() => { if (!canEdit) { onToast("当前角色只能查看通知设置；请切换为老板后编辑"); return; } setValues((current) => current.map((value, itemIndex) => itemIndex === index ? !value : value)); }} />)}</div><button className="outline-button" disabled={!canEdit} onClick={() => onToast(canEdit ? "通知设置已保存" : "当前角色只能查看通知设置；请切换为老板后保存")}><CheckCircle size={16} />{canEdit ? "保存通知设置" : "老板可编辑"}</button></SectionCard>; }
