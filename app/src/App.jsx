...
function HistoryView({ role, query, records: sourceRecords = historyRecords, onToast, onOpenRecord }) {
  ...
  const records = useMemo(() => sourceRecords.filter((record) => {
    ...
    const searchable = [record.title, record.description, record.summary, record.applications.join(" "), record.resources.map((item) => item.name).join(" ")].join(" ").toLowerCase();
