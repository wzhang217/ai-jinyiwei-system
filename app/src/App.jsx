...
function App() {
  ...
  useEffect(() => {
    if (!agentApiEnabled || !principal) return undefined;
    let cancelled = false;
    getLiveOrganization().then((organization) => {
      if (!cancelled && organization?.name) setOrganizationName(organization.name);
    }).catch(() => {
      // The page can still operate with the neutral fallback while the
  ...
  return <div className="shell">
    ...
    <main className="main-content">
      ...
      {activeNav === "history" ? <HistoryView role={role} principal={principal} organizationName={organizationName} query={query} records={liveRecords ?? (demoMode ? historyRecords : [])} lastUpdatedAt={liveUpdatedAt} refreshing={liveRefreshing} liveError={liveError} onRefresh={refreshLiveRecords} onToast={notify} onOpenRecord={setSelectedRecord} onNavigate={navigate} /> : <AdminPage page={activeNav} role={role} principal={principal} query={query} target={navigationTarget} liveRecords={liveRecords} onNavigate={navigate} onToast={notify} />}
    </main>
...
function HistoryView({ role, principal, organizationName, query, records: sourceRecords = [], lastUpdatedAt, refreshing, liveError, onRefresh, onToast, onOpenRecord, onNavigate }) {
