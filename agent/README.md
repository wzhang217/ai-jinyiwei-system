...
企业代理环境可在启动 Agent 前设置当前 Windows 用户或企业分发环境变量 `AGENT_PROXY_URL`，例如 `http://proxy.example.com:8080`。不需要代理时不要设置；不要把包含密码的代理 URL 写入安装包或 Git 仓库。
