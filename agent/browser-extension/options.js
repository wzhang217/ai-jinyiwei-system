...
async function load() {
  ...
  if (config.browserToken) show(`浏览器已配对，有效至 ${config.browserTokenExpiresAt ? new Date(config.browserTokenExpiresAt).toLocaleString("zh-CN") : "服务端返回的时间"}。如需重新配对，输入新配对码后保存。`);
}
